...
namespace Alcor\Payroll\Tests\Infrastructure;

use Alcor\Payroll\Application\ConcurrentStreamWrite;
use Alcor\Payroll\Application\EarningLineNotFound;
...
final class SqliteEventStoreTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->repository = new EarningLineRepository($this->store);
    }
    ...
    public function testEventsSurviveReopeningTheDatabaseWithExactAmountsAndAuditMetadata(): void
    {
        ...
        $reopened = new EarningLineRepository(new SqliteEventStore(new PDO('sqlite:' . $this->database), new EventSerializer()));
    ...
    public function testAStaleWriterCannotOverwriteAnAcceptedChange(bool $adjustmentWins): void
    {
        ...
        $secondRepository = new EarningLineRepository(new SqliteEventStore(new PDO('sqlite:' . $this->database), new EventSerializer()));
